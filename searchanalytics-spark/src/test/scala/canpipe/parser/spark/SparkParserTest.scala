...
class SparkParserTest extends FlatSpec with BeforeAndAfter {
  ...
  it should "not have event ids fields left empty" in {
    ...
    resourceFileNamesAndNumberOfEvents.foreach {
      case (fileInfo, _) =>
        ...
        val r = myParser.parseEventGroup(events = EventGroupFromHDFSFile(sc, hdfsFileName))
        // r.persist()
        // val invalidEntries = r.filter(e => e.eventId.isEmpty || e.eventTimestamp.isEmpty).collect() // .toSet
        // val eventsWithEmptyId = invalidEntries.filter(_.eventId.isEmpty).size
        val eventsWithEmptyId = r.filter(_.eventId.isEmpty).collect().length
        withClue(s"Parsing '${fileInfo.name}': ${eventsWithEmptyId} have empty 'eventId'") { assert(eventsWithEmptyId == 0) }
      // val eventsWithEmptyTimestamp = invalidEntries.filter(_.eventTimestamp.isEmpty).size
      // val eventsWithEmptyTimestamp = r.filter(_.eventId.isEmpty).collect().length
      // withClue(s"${eventsWithEmptyTimestamp} have empty 'eventTimestamp' ") { assert(eventsWithEmptyTimestamp == 0) }
    }
  ...
  it should "not have timestamp fields left empty" in {
    val testName = "my test 2"
    val sc = new SparkContext("local[4]", testName)
    val myParser = new SparkParser()
    ...
    resourceFileNamesAndNumberOfEvents.foreach {
      case (fileInfo, _) =>
        ...
        val hdfsFileName = resourceFileName2HDFSFileName(fileInfo.name)
        val r = myParser.parseEventGroup(events = EventGroupFromHDFSFile(sc, hdfsFileName))
        val eventsWithEmptyTimestamp = r.filter(_.eventTimestamp.isEmpty).collect().length
        withClue(s"${eventsWithEmptyTimestamp} have empty 'eventTimestamp' ") { assert(eventsWithEmptyTimestamp == 0) }
    }
